...
class Attention(nn.Module):
    def __init__(self, nx, n_positions, config, scale=False, is_cross_attention=False):
        ...
        # print(f'Attention.__init__() nx {nx}')
        # print(f'Attention.__init__() n_positions {n_positions}')
        ...
        # print(f'Attention.__init__() self.bias.shape {self.bias.shape}')
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, output_attentions=False):
        ...
        if not self.is_cross_attention:
            ...
            # print(f'Attention._attn() applying causal mask?')
            # print(f'Attention._attn() bias.shape {self.bias.shape}')
            mask = self.bias[:, :, ns - nd: ns, :ns]
            # print(f'Attention._attn() mask.shape {mask.shape}')
            w = torch.where(mask.bool(), w, self.masked_bias.to(w.dtype))
            # print(f'Attention._attn() w.shape {w.shape}')

        if attention_mask is not None:
            # Apply the attention mask
            # print(f'Attention._attn() applying attention mask {attention_mask.shape}')
            w = w + attention_mask
        ...
        if head_mask is not None:
            assert False
            print(f'Attention._attn() applying head mask')
            w = w * head_mask

        outputs = [torch.matmul(w, v)]
        if output_attentions:
            assert False
            print(f'Attention._attn() returning attentions')
            outputs.append(w)
        return outputs

    def merge_heads(self, x):
        # print(f'Attention.merge_heads() input x.shape {x.shape}')
        x = x.permute(0, 2, 1, 3).contiguous()
        new_x_shape = x.size()[:-2] + (x.size(-2) * x.size(-1),)
        # print(f'Attention.merge_heads() new_x_shape {new_x_shape}')
        return x.view(*new_x_shape)  # in Tensorflow implem: fct merge_states

    def split_heads(self, x, k=False):
        # print(f'Attention.split_heads() input x.shape {x.shape}')
        new_x_shape = x.size()[:-1] + (self.n_head, x.size(-1) // self.n_head)
        # print(f'Attention.split_heads() new_x_shape {new_x_shape}')
    ...
    def forward(
        ...
        if encoder_hidden_states is not None:
            ...
            # print(f'Attention.forward() conv1d c_attn({hidden_states.size()}) -> query,key,value')
            c_attn = self.c_attn(hidden_states)
            # print(f'GPT2Attention.call() c_attn.shape {c_attn.shape}')
            query, key, value = c_attn.split(self.split_size, dim=2)
            # print(f'Attention.forward() query.size {query.size()} | key.size {key.size()} | value.size {value.size()}')

        query = self.split_heads(query)
        key = self.split_heads(key, k=True)
        value = self.split_heads(value)
        # print(f'Attention.forward() split query.size {query.size()} | key.size {key.size()} | value.size {value.size()}')
        if layer_past is not None:
            assert False
            print(f'Attention.forward() layer_past.size {layer_past.size()}')
...
class Block(nn.Module):
    ...
    def forward(
        ...
        # print(f'Block.forward() x {hidden_states}')
...
class GPT2Model(GPT2PreTrainedModel):
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            # print(f'GPT2Model.forward input attention_mask.shape {attention_mask.shape}')
            attention_mask = attention_mask.view(batch_size, -1)
            # print(f'GPT2Model.forward reshape 1 attention_mask.shape {attention_mask.shape}')
            ...
            # print(f'GPT2Model.forward reshape 2 attention_mask.shape {attention_mask.shape}')
